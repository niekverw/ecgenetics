...
get_genes_df <- function(df,chr="CHR",position="BP",snp="uniqid"){
  ...
  df.anno.agg <- merge(df.anno.band,df.anno.gene,by="snp")
  names(df.anno.agg) <- c(snp,"band","Gene")
  df.genes <- plyr::join(df[,unique(colnames(df))],df.anno.agg,by=snp ) # deduplicate, sometimes uniqid is twice
...
# 
# df <- fread("/Users/niekverw/Downloads/snps_ecg.txt")
# df <- unique(df)
# df <- get_genes_df(df,chr="CHR",position="BP",snp="SNP")
# fwrite(df,"/Users/niekverw/Downloads/snps_ecg.txt.anno",file=,sep="\t",quote = FALSE)
